...
st.title('La place de Rousset dans la métropole Aix-Marseille-Provence')
#st.title('Outils pour la Ville de Rousset')
# fonction pour l'affichage des caractéristiques des fichiers
...
* **Outils développés pour  :** la Ville de Rousset
* **Auteur :** olivier Boylaud  (olivier@boylaud.eu)
* **Source des données : ** DGCL et DGFIP.
""")
st.write(' ')
compar = st.sidebar.radio("SELECTIONNER",("Remise à zéro",'Cartographie fiscalité par habitant', "Dynamique de Rousset dans la Métropole",'Cartographie autres données', "Révisions des attributions de compensation"))
if compar == 'Remise à zéro"' :
    st.write('')
if compar == 'Cartographie fiscalité par habitant' :
    #st.sidebar.header('Les ressources par commune')
    with st.sidebar.beta_expander("Aide"):
            st.write("""
    ...
    kl = st.sidebar.number_input("Choix du nombre de groupes", min_value=1, value=7, step=1)
    method = st.sidebar.selectbox('Choix de la méthode pour la construction des groupes',('Seuils naturels', 'Quantiles','Intervalle égal'))
    villes = st.sidebar.selectbox('Affichage des villes', (('Toutes','Uniquement les principales')))
    if st.sidebar.button('Construire la carte') :
        #st.write(" ")
        #st.write("Construction de la carte..")
        #st.write(" ")
        #indic = 'indic'
        titre = indic
        source = "Source : calculs sur données DGCL"
        #kl = 7 #nombre de classes
        ...
        if villes == 'Uniquement les principales':
        ...
        with st.beta_expander("Les données"):
                st.dataframe(tab)
if compar == "Dynamique de Rousset dans la Métropole" :
    #fichier_source = 'data_cartes/ressources_mpm2016_2021.ods' 
    #ressource_mpm1 = pd.read_csv('data_cartes/ressources_mpm2016_2021.csv',sep=",")
    #with st.beta_expander("Ressource de la Métropole"):
    #st.header('Les ressources de la Métropole')
    #st.dataframe(ressource_mpm1)
    #st.write('2021 : estimation débat orientation budgétaire')
    ressources = pd.read_csv('data_cartes/ressources_mpm2016_2021.csv',sep=";")
    #liste_indic = list(ressources)
    #del liste_indic[0:3]
    ...
    #ressources['IFER']= ressources['IFER'].astype(int)
    ...
    #liste_indic_eco = list(ressources)
    #del liste_indic_eco[0:3]
    #del liste_indic_eco[6:12]
    #print(liste_indic)
    #print(liste_indic_eco)
    ...
    #print(ressources_b100_2017)
    #ressources
    #a = st.sidebar.selectbox("Choix de l'indicateur",('CFE', 'CVAE', 'TAFNB', 'IFER', 'TASCOM', 'DCRTP_GIR', 'Fiscalité économique (y compris DCRTP et GIR)','FNB','FB','TH','TVA'))
    ...
    #sns.lineplot(x = 'année', y = var1,ax=ax,data=tab_data)
    #plt.bar(x = 'année', y = var1,ax=ax,data=tab_data)   
    st.set_option('deprecation.showPyplotGlobalUse', False)
    #tab_data[var1].plot()
    plt.title(a+' : '+var1,size=18)
    plt.ylabel("en % de la Métropole",size=14)
    ...
    plt.title(a+' : Comparaison Rousset vs Métropole',size=18)
    plt.ylabel("base 100 en 2017",size=14)
    plt.xlabel("année",size=14)
    st.pyplot()
    st.write(' ')
...
if compar == "Cartographie autres données" :  
    indic = st.sidebar.selectbox('Autres données',("Attribution de compensation de la commune divisé par Recettes réelles de fonctionnement",'Potentiel financier par hab. divisé par le Potentiel financier par hab. moyen de la métropole','Revenu imposable par hab.','Nombre logements sociaux de la commune divisé par Nombre de logements TH de la commune'))
    kl = st.sidebar.number_input("Nombre de groupes", min_value=1, value=7, step=1)
    method = st.sidebar.selectbox('Méthode pour la construction des groupes',('Seuils naturels', 'Quantiles','Intervalle égal'))
    villes = st.sidebar.selectbox('Affichage des villes', (('Toutes','Principales')))
    if st.sidebar.button('Valider votre choix'):
       	titre = indic
       	source = "Source : calculs sur données DGCL"
       	if method =='Seuils naturels':
            m = mapclassify.NaturalBreaks
        ...
        if villes == 'Toutes' :
            ensemble_des_communes()
        if villes == 'Principales':
            principales_communes()
        #plt.savefig(indic+'.jpg')
        #f.savefig(indic+".pdf", bbox_inches='tight')
        st.pyplot(f)
        #img = mpimg.imread('Rousset_CAPA.png')
        #st.pyplot(img)
        mycol=['codgeo','Nom Collectivité',indic]
        tab = data_carte[mycol]
        with st.beta_expander("Les données"):
...
if compar == "Révisions des attributions de compensation":
	with st.beta_expander("Révision des attributions de compensation"):
		st.write("""
...
montant des transferts de charges opérés entre l’EPCI et la commune
         		""")
	compar2 = st.sidebar.radio("Choisir le type de simulation",("Révision individualisée","Révision unilatérale"))
...
expander_bar = st.sidebar.beta_expander("En savoir plus")
expander_bar.markdown("""
* L’application est 
    * Un outil de cartographie permettant de comparer Rousset avec les autres communes de la Métropole,
    * Un outil de simulation pour tester l’impact d’une réforme des allocations de compensation,
    * Un outil pour accéder aux ressources de la Métropole,
 * L’application repose sur des données provenant de la DGCL (critères utilisées pour le calcul des dotations) et de la DGFIP (fichier de recensement des éléments d'imposition à la fiscalité directe locale -REI-). 
""")
