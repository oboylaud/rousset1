...
def cartographie():
    ...
    #indic = st.sidebar.selectbox("Choix de l'indicateur",('CVAE par habitant','CFE par habitant','CET par habitant','TASCOM par habitant','IFER par habitant','TAFNB par habitant', 'Bases CFE par habitant','Bases brutes de FB par habitant','Bases brutes de FNB par habitant','Bases brutes de TH par habitant', 'Bases nettes FB par habitant','Bases nettes FNB par habitant','Bases nettes TH par habitant','ifer_pc','Tascom_pc', 'CVAE par habitant -total-','CET par habitant -total-',"Attribution de compensation de la commune divisé par Recettes réelles de fonctionnement",'Potentiel financier par hab. divisé par le Potentiel financier par hab. moyen de la métropole','Revenu imposable par hab.','Nombre logements sociaux de la commune divisé par Nombre de logements TH de la commune'))
    kl = st.sidebar.number_input("Choix du nombre de classes", min_value=1, value=7, step=1)
    method = st.sidebar.selectbox('Choix de la méthode pour la construction des classes',('Seuils naturels', 'Quantiles','Intervalle égal'))
    villes = st.sidebar.selectbox('Affichage du nom des communes', (('Toutes les communes','Uniquement les principales')))
...
#st.title('Que voulez-vous faire ?')
...
st.write(' ')
st.write(' ')
choix = st.selectbox("Que voulez-vous faire ?",("choisir l'option dans le menu déroulant",'Comparer Rousset avec les autres communes de la Métropole (cartographie)',"Comparer l'évolution de Rousset avec la Métropole","Mesurer l'impact d'une révision de l'allocation de compensation"))
#if st.button('Valider'):
# module cartographie
if choix == 'Comparer Rousset avec les autres communes de la Métropole (cartographie)':
    cartographie()

# module dynamique de Rousset
if choix == "Comparer l'évolution de Rousset avec la Métropole":
    st.title("Comparer l'évolution de Rousset avec l'évolution de la Métropole")
